...
# -*- coding: utf-8 -*-
"""
Generate BYO variant pool count data
"""
...
from k_seq.data.byo_doped import load_byo_doped
from k_seq.utility.file_tools import dump_pickle, check_dir


def main():
    dataset = load_byo_doped(from_count_file=True, count_file_path=args.count_file_dir, norm_path=args.norm_file)
    dump_pickle(dataset, args.output)


if __name__ == '__main__':
    parser = ArgumentParser(
        prog="Generate BYO variant-pool dataset",
        description="Generate a pickled BYO variant-pool dataset from preprocessed count files",
